...
fn main() {
    ...
    (0..n).for_each(|r| {
        (0..m).for_each(|c| {
            let check = board[r..r + BOARD_SIZE].iter().flat_map(|row| row[c..c + BOARD_SIZE].chars());
            let w_diff = check.clone().zip(W_BOARD.chars()).filter(|(a, b)| a != b).count();
            let b_diff = check.clone().zip(B_BOARD.chars()).filter(|(a, b)| a != b).count();
